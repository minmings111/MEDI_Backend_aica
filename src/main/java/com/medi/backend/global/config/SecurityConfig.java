    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http
                ...
                .authorizeHttpRequests(auth -> auth
                        .requestMatchers("/health").permitAll() // ELB health check
                        .requestMatchers("/actuator/health").hasRole("ADMIN") // Actuator health
                                                                              // check (Admin
                                                                              // only)
                        ...
                        .requestMatchers("/api/auth/send-password-reset").permitAll() // 비밀번호
                                                                                      // 재설정 코드
                                                                                      // 전송
                        ...
                        .requestMatchers("/api/v1/analysis/**").permitAll() // AI 서버에서 POST로 결과
                                                                            // 전송
                        .requestMatchers("/api/agent/**").permitAll() // FastAPI Agent 요청 전체 허용
                        .requestMatchers("/api/filter/prompt/**").permitAll() // 에이전트용 프롬프트 조회
                                                                              // API
                        .requestMatchers("/api/youtube/analysis/channel/save").permitAll() // FastAPI
                                                                                           // Agent에서
                                                                                           // 분석
                                                                                           // 결과
                                                                                           // 저장
                ...
                .oauth2Login(oauth2 -> oauth2
                        ...
                        .authorizationEndpoint(authorization -> authorization
                                .authorizationRequestRepository(
                                        authorizationRequestRepository()) // OAuth2
                                                                          // 인증
                                                                          // 요청
                                                                          // 저장소
                                                                          // 명시적
                                ...
                                .authorizationRequestResolver(
                                        customOAuth2AuthorizationRequestResolver()) // 커스텀
                                                                                    // 리졸버
                                                                                    // 적용

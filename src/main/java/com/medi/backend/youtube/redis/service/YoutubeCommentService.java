...
/**
 * 조회수 상위 10개 영상의 댓글을 Redis에 저장하는 서비스
 */
public interface YoutubeCommentService {

    /**
     * 각 채널별 조회수 상위 10개 영상의 댓글 동기화
     * 
     ...
     */
    long syncTop10VideoComments(Integer userId, Map<String, List<RedisYoutubeVideo>> videosByChannel,
            SyncOptions options);

    /**
     * 초기 동기화용 편의 메서드 (기본 옵션 사용)
     * 
     * @param userId          사용자 ID
     * @param videosByChannel 채널별 비디오 리스트
     * @return 저장된 댓글 개수
     */
    default long syncTop10VideoComments(Integer userId, Map<String, List<RedisYoutubeVideo>> videosByChannel) {
        return syncTop10VideoComments(userId, videosByChannel, SyncOptions.initialSync());
    }

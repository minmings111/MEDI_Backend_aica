...
public class ReportController {
    ...
    @PostMapping("/legal")
    @Operation(summary = "합법 보고서 생성 요청", description = "합법 보고서 생성을 위한 작업을 큐에 추가합니다.")
    public ResponseEntity<Map<String, Object>> createLegalReport(@RequestBody ReportRequest request) {
        ...
        try {
            ...
            log.info("✅ 합법 보고서 생성 요청: userId={}, channelId={}", userId, request.getChannelId());
            
            return ResponseEntity.ok(response);
        } catch (Exception e) {
            log.error("❌ 합법 보고서 생성 요청 실패: userId={}, channelId={}", userId, request.getChannelId(), e);
            Map<String, Object> errorResponse = new HashMap<>();
            errorResponse.put("message", "Failed to add legal report task to queue");
    ...
    @PreAuthorize("isAuthenticated()")
    @PostMapping("/content")
    @Operation(summary = "콘텐츠 보고서 생성 요청", description = "콘텐츠 보고서 생성을 위한 작업을 큐에 추가합니다.")
    public ResponseEntity<Map<String, Object>> createContentReport(@RequestBody ReportRequest request) {
        Integer userId = authUtil.getCurrentUserId();
        if (userId == null) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
        }
        ...
        try {
            ...
            log.info("✅ 콘텐츠 보고서 생성 요청: userId={}, channelId={}", userId, request.getChannelId());

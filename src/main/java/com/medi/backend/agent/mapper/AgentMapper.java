...
public interface AgentMapper {
        ...
        /**
         * 댓글 소유권 확인 (사용자의 채널에 속하는지)
         * 
         * @param userId           사용자 ID
         * @param youtubeCommentId YouTube 댓글 ID
         * @return 소유권이 있으면 1, 없으면 0
         */
        Integer checkCommentOwnership(
                        @Param("userId") Integer userId,
                        @Param("youtubeCommentId") String youtubeCommentId);
         ...
         * 
         * @param youtubeCommentId      YouTube 댓글 ID
         * @param youtubeDeletionStatus YouTube API 삭제 결과 (SUCCESS, NOT_FOUND, FAILED)
         * @return 업데이트된 행 수
         */
        Integer updateCommentStatusToDeleted(
                        @Param("youtubeCommentId") String youtubeCommentId,
                        @Param("youtubeDeletionStatus") String youtubeDeletionStatus);

        /**
         * 비동기 삭제: 비디오의 필터링된 댓글을 PENDING_DELETE 상태로 변경
         * 
         * @param videoId   비디오 ID
         * @param requestId 삭제 요청 ID (UUID)
         * @return 업데이트된 행 수
         */
        Integer markCommentsForDeletion(
                        @Param("videoId") Integer videoId,
                        @Param("requestId") String requestId);

        /**
         * 비동기 삭제: 채널의 필터링된 댓글을 PENDING_DELETE 상태로 변경
         * 
         ...
         * @return 업데이트된 행 수
         */
        Integer markChannelCommentsForDeletion(
                        @Param("channelId") Integer channelId,
                        @Param("userId") Integer userId,
                        @Param("requestId") String requestId);
         ...
         * @return 처리할 댓글 리스트 (Map 형태)
         */
        List<Map<String, Object>> findPendingDeletionComments(
         ...
         * @param nextRetryAt      다음 재시도 시간
         * @return 업데이트된 행 수
         */
        Integer incrementDeletionRetryCount(
                        @Param("youtubeCommentId") String youtubeCommentId,
         ...
         * @return 진행 상황 (totalComments, completedComments, failedComments)
         */
        Map<String, Object> getDeletionJobProgress(@Param("requestId") String requestId);
}
